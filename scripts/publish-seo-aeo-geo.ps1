...
if (faqGraph['@type'] !== 'FAQPage' || faqGraph.mainEntity.length !== 5) throw new Error('faq.jsonld missing 5 FAQ answers');
...
if (!Array.isArray(answers.answers) || answers.answers.length < 5) throw new Error('answers.json missing verified answers');
